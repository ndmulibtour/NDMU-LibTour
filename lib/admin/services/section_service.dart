...
class SectionService {
  ...
  // Add new section
  // Task 1: added optional sceneId parameter
  Future<bool> addSection({
    ...
    try {
      ...
      await _firestore.collection(_collection).add({
        ...
        'imageUrl': imageUrl,
        'order': order,
        'sceneId': sceneId?.isEmpty == true ? null : sceneId, // Task 1
        'createdAt': now,
        'updatedAt': now,
      });
  ...
  // Update section
  // Task 1: sceneId is written via section.toMap() automatically
  Future<bool> updateSection(LibrarySection section) async {
    try {
      await _firestore.collection(_collection).doc(section.id).update({
        ...
        'order': section.order,
        'sceneId':
            section.sceneId?.isEmpty == true ? null : section.sceneId, // Task 1
